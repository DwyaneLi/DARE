...
StartDare() {
    ...
    for ((i=0, j=0; i<$1; j++)); do
        ...
        run_dare=( "${DAREDIR}/bin/srv_test" "-l $PWD/debug_srv${i}.log" "-n ${nodes[$j]}" "-s $1" "-i $i" "-m $DGID" )
...
#cmd=( "${DAREDIR}/bin/clt_test" "--rtrace" "-t $trace_file" "-o $data_file" "-l debug_clt.log" "-m $DGID")
#echo "Executing: ${cmd[@]}"
#${cmd[@]}
